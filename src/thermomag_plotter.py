...
def plot_data(processed_lt_data, processed_ht_data, processed_lt2_data, plot_filename):
    """This function plots the processed data using 
    colors/linestyles/etc. commonly used in paleomagnetic research.""" 
    ...
    ## Adding arrows to indicate direction of measurement - This will require tailoring by the user
    ...
    ## Adding title, axes labels, and legend - This should also be tailored by the user
...
def plot():
    ...
    lt_data = read_data(lt_input_filename)
    ht_data = read_data(ht_input_filename)
    lt2_data = read_data(lt2_input_filename)
    
    processed_lt_data = process_data(lt_data)
    processed_ht_data = process_data(ht_data)
    processed_lt2_data = process_data(lt2_data)
    plot_data(processed_lt_data, processed_ht_data, processed_lt2_data, plot_filename)  
